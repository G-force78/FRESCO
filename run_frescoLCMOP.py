...
def run_keyframe_translation(config):
    ...
    if "Realistic" in config["sd_path"] or "realistic" in config["sd_path"]:
        a_prompt = "soft lighting, high quality, film grain, Fujifilm XT3, "
    ...
    keys = get_keyframe_ind(
        config["file_path"], frame_num, config["mininterv"], config["maxinterv"]
    )
    ...
    sublists = [
        keys[i : i + config["batch_size"] - 2]
        for i in range(2, len(keys), config["batch_size"] - 2)
    ]
    ...
    for i in range(frame_num):
        ...
        Image.fromarray(img).save(
            os.path.join(config["save_path"], "video/%04d.jpg" % (i))
        )
        ...
        print(
            "processing batch [%d/%d] with %d frames"
            % (batch_ind + 1, len(sublists), len(sublists[batch_ind]))
        )
        ...
        prompts = [
            base_prompt + a_prompt + extra_prompts[ind] for ind in sublists[batch_ind]
        ]
        ...
        edges = torch.cat(
            [
                numpy2tensor(apply_control(img, detector, config)[:, :, None])
                for img in imgs
            ],
            dim=0,
        )
        ...
        correlation_matrix = get_intraframe_paras(
            pipe, imgs_torch, frescoProc, prompt_embeds, seed=config["seed"]
        )
        ...
        * Turn off background smoothing: set saliency = None in apply_FRESCO_opt()
        ...
        frescoProc.controller.enable_controller(
            interattn_paras=interattn_paras, attn_mask=attn_mask
        )
        apply_FRESCO_opt(
            pipe,
            steps=timesteps[: config["end_opt_step"]],
            flows=flows,
            occs=occs,
        ...
        with torch.no_grad():
            ...
            for ind, num in enumerate(sublists[batch_ind]):
                Image.fromarray(save_imgs[ind + bias]).save(
                    os.path.join(config["save_path"], "keys/%04d.jpg" % (num))
                )
