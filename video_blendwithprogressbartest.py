#modified with code from claude 3 sonnet
...
def process_sequences(i_arr, video_sequence: VideoSequence):
    for i in tqdm.tqdm(i_arr, desc="Processing sequences", total=len(i_arr)):
...
def process_seq(video_sequence: VideoSequence,
    ...
    for i in tqdm.tqdm(range(interval - 1), desc="Processing frames", total=interval - 1):
        c_id = beg_id + i + 1
...
def main(args):
    ...
    for i in tqdm.tqdm(range(video_sequence.n_seq), desc="Processing sequences", total=video_sequence.n_seq):
