...
def get_models(config):
    ...
    #return pipe, frescoProc, controlnet, detector, flow_model, sod_model, ip_model, ip_model_plus
...
def process1(input_path, prompt, sd_model, seed, image_resolution, control_strength, 
            ...
            b1, b2, s1, s2, preserve_face_structure, face_strength, likeness_strength):
    global global_state
...
def process2(input_path, prompt, sd_model, seed, image_resolution, control_strength, 
    ...
    for img in imgs:
        ...
        prompt=prompts, negative_prompt=negative_prompts, faceid_embeds=average_embedding,
        scale=likeness_strength, face_image=face_image, shortcut=True, s_scale=face_strength,
        num_samples=1, seed=seed, guidance_scale=guidance_scale, width=512, height=512,
        num_inference_steps=ddpm_steps
    )
    else:
        image = ip_model.generate(
        prompt=prompts, negative_prompt=negative_prompts, faceid_embeds=average_embedding,
        scale=likeness_strength, width=512, height=512, num_inference_steps=ddpm_steps
    )
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion('IP Adapter Settings', open=False):
                preserve_face_structure = gr.Checkbox(label='Preserve Face Structure', value=True)
                face_strength = gr.Slider(label='Face Structure Strength', minimum=0.0, maximum=3.0, value=1.3, step=0.1)
                likeness_strength = gr.Slider(label='Face Embed Strength', minimum=0.0, maximum=5.0, value=1.0, step=0.1)
    ...
    run_button.click(fn=process,
                     inputs=ips, preserve_face_structure, face_strength, likeness_strength,
                     outputs=[result_keyframe, result_video])
    run_button1.click(fn=process1, inputs=ips, preserve_face_structure, face_strength, likeness_strength, outputs=[result_keyframe])
    run_button2.click(fn=process2, inputs=ips, preserve_face_structure, face_strength, likeness_strength, outputs=[result_video])
